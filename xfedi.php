...
if (!extension_loaded('pdo_sqlite')) {
    die('<h2>XFedi requires the PDO SQLite extension. Please enable <code>pdo_sqlite</code> in your PHP configuration.</h2>');
}
...
function baseURL(): string {
    ...
    $path  = $_SERVER['SCRIPT_NAME'] ?? '/xfedi.php';
...
function avatarEl(string $avatarUrl, string $name, int $size = 40): string {
    ...
    $fallback = '<div class=\'avatar-placeholder\' style=\'' . $style . 'font-size:' . $fs . 'px\'>' . $initials . '</div>';
    if ($avatarUrl) {
        return '<img src="' . h($avatarUrl) . '" class="avatar" style="' . $style . '" alt="' . h($name) . '" loading="lazy"'
             . ' onerror="this.outerHTML=\'' . addslashes($fallback) . '\'">';
    }
    return '<div class="avatar-placeholder" style="' . $style . 'font-size:' . $fs . 'px" aria-label="' . h($name) . '">' . $initials . '</div>';
}

function resolveAbsoluteUrl(string $url, string $base): string {
    if (!$url) return '';
    ...
    foreach ($handles as $handle) {
        ...
        foreach ($data['posts'] as $p) {
            ...
            if (!empty($p['author']['avatar'])) {
                $p['author']['avatar'] = resolveAbsoluteUrl($p['author']['avatar'], $base);
            }
            $rid = (string) $p['id'];
            $chk->execute([$handle, $rid]);
            $existing = $chk->fetch();
            if ($existing) {
                $old = json_decode($existing['post_json'], true) ?? [];
                if (($old['likes'] ?? 0)         !== ($p['likes'] ?? 0)
                 || ($old['content'] ?? '')       !== ($p['content'] ?? '')
                 || ($old['updated_at'] ?? 0)     !== ($p['updated_at'] ?? 0)
                 || ($old['comment_count'] ?? 0)  !== ($p['comment_count'] ?? 0)) {
                    $upd->execute([json_encode($p), $handle, $rid]);
                }
            } else {
                $ins->execute([$handle, $rid, json_encode($p)]);
            }
        }
        if (!empty($data['profile'])) {
            $av = resolveAbsoluteUrl($data['profile']['avatar'] ?? '', $base);
            db()->prepare("UPDATE follows SET name=?,avatar=? WHERE handle=?")->execute([$data['profile']['name'] ?? '', $av, $handle]);
        }
    }
}
...
if (isset($_GET['api'])) {
    ...
    if ($api === 'profile') {
    ...
    } elseif ($api === 'followers') {
        $limit = min((int) ($_GET['limit'] ?? 20), 50);
        jsonOut([
            'followers' => db()->query("SELECT handle,name,avatar,created_at FROM followers ORDER BY created_at DESC LIMIT $limit")->fetchAll(),
            'count'     => (int) db()->query("SELECT COUNT(*) FROM followers")->fetchColumn(),
        ]);
    } elseif ($api === 'posts') {
        $since = (int) ($_GET['since'] ?? 0);
        $limit = min((int) ($_GET['limit'] ?? 20), 50);
        $rows  = db()->prepare("SELECT * FROM posts WHERE created_at > ? ORDER BY created_at DESC LIMIT ?");
        $rows->execute([$since, $limit]);
        $posts = $rows->fetchAll();
        foreach ($posts as &$p) {
            ...
            $cs = db()->prepare("SELECT COUNT(*) FROM comments WHERE post_id=?");
            $cs->execute([$p['id']]);
            $p['comment_count'] = (int) $cs->fetchColumn();
            unset($p['image_path']);
        }
        unset($p);
        jsonOut(['posts' => $posts, 'profile' => ['name' => $u['name'] ?? '', 'avatar' => $u['avatar'] ? $base . '?file=avatar' : '']]);
    } elseif ($api === 'post') {
        $id = (int) ($_GET['id'] ?? 0);
        ...
        $p['image_url'] = $p['image_path'] ? $base . '?file=post_img&id=' . $p['id'] : '';
        unset($p['image_path']);
        $cs = db()->prepare("SELECT * FROM comments WHERE post_id=? ORDER BY created_at ASC");
        $cs->execute([$id]);
        ...
        if (!$b) jsonOut(['error' => 'bad body'], 400);
        $type   = $b['type']   ?? '';
        ...
        try {
            if ($type === 'like') {
                ...
                $cc = db()->prepare("SELECT COUNT(*) FROM comments WHERE post_id=?");
                $cc->execute([$postId]);
                jsonOut(['ok' => true, 'likes' => (int) db()->query("SELECT likes FROM posts WHERE id=$postId")->fetchColumn(), 'comment_count' => (int) $cc->fetchColumn()]);
            }
            if ($type === 'comment') {
                $c = substr(strip_tags($b['content'] ?? ''), 0, 500);
                if (!$c) jsonOut(['error' => 'empty'], 400);
                ...
                $cs = db()->prepare("SELECT * FROM comments WHERE post_id=? ORDER BY created_at ASC");
                $cs->execute([$postId]);
                $lk = db()->query("SELECT likes FROM posts WHERE id=$postId")->fetchColumn();
                jsonOut(['ok' => true, 'comments' => $cs->fetchAll(), 'likes' => (int) $lk, 'comment_count' => count($cs->fetchAll())]);
            }
            if ($type === 'follow') {
...
if (isset($_GET['file'])) {
    ...
    if ($key === 'post_img') {
        $s = db()->prepare("SELECT image_path FROM posts WHERE id=?");
        $s->execute([(int) ($_GET['id'] ?? 0)]);
        $row = $s->fetch();
        $serve($row['image_path'] ?? '');
    }
...
if ($isFirstRun && ($_POST['action'] ?? '') === 'setup') {
    ...
    if (!preg_match('/^[a-z0-9_]{3,30}$/', $uname)) {
        $errors[] = 'Username must be 3–30 chars: lowercase, numbers, underscores.';
    } else {
...
if ($act === 'login') {
    if ($cu && trim($_POST['username'] ?? '') === $cu['username'] && password_verify($_POST['password'] ?? '', $cu['password'])) {
        session_regenerate_id(true);
        $_SESSION['auth'] = true;
        redir($base);
    }
...
if ($act === 'create_post' && isLoggedIn() && checkCsrf()) {
    ...
    if ($type === 'image' && !empty($_FILES['image']['name'])) {
        ...
        if (!$n) $errors[] = 'Invalid image (JPEG/PNG/GIF/WebP, max 10 MB).';
        else $imgName = $n;
    }
    if (!$errors) {
        db()->prepare("INSERT INTO posts(type,content,image_path) VALUES(?,?,?)")->execute([$type, $content, $imgName]);
        redir($base);
    }
}
...
if ($act === 'delete_post' && isLoggedIn() && checkCsrf()) {
    ...
    if ($row) {
        ...
        foreach (['posts', 'likes', 'comments'] as $t)
            db()->prepare("DELETE FROM $t WHERE " . ($t === 'posts' ? 'id' : 'post_id') . "=?")->execute([$id]);
    }
...
if ($act === 'comment' && isLoggedIn() && checkCsrf()) {
    $id   = (int) ($_POST['post_id'] ?? 0);
    $text = mb_substr(trim($_POST['comment'] ?? ''), 0, 500);
...
if ($act === 'follow' && isLoggedIn() && checkCsrf()) {
    ...
    if (preg_match('/^@[^@\s]+@[^\s]+$/', $fh)) {
        $p    = parseHandle($fh);
        $prof = $p ? fedGet($p, 'api=profile') : null;
        $av   = '';
        if ($prof && !empty($prof['avatar'])) {
            $av = resolveAbsoluteUrl($prof['avatar'], $p['url_https']);
        }
        db()->prepare("INSERT OR IGNORE INTO follows(handle,name,avatar) VALUES(?,?,?)")->execute([$fh, $prof['name'] ?? '', $av]);
        db()->prepare("UPDATE follows SET name=?,avatar=? WHERE handle=?")->execute([$prof['name'] ?? '', $av, $fh]);
        $myHandle = selfHandle();
        $myAv     = $cu['avatar'] ? $base . '?file=avatar' : '';
...
if ($act === 'update_profile' && isLoggedIn() && checkCsrf()) {
    ...
    $links = array_values(array_filter(array_map('trim', (array) ($_POST['links'] ?? [])), function ($l) { return $l && filter_var($l, FILTER_VALIDATE_URL); }));
    $setCols = ['name=?', 'bio=?', 'links=?'];
    $setVals = [$name, $bio, json_encode($links)];
    $cf      = getUser(true);
    foreach (['avatar', 'cover'] as $field) {
        if (!empty($_FILES[$field]['name'])) {
            $n = safeUpload($_FILES[$field], $field);
            if ($n) {
                if ($cf[$field] && file_exists(UPLOAD_DIR . $cf[$field])) @unlink(UPLOAD_DIR . $cf[$field]);
                $setCols[] = "$field=?";
                $setVals[] = $n;
            } else {
    ...
    if (!$errors && $name) {
        $setVals[] = 1;
        db()->prepare("UPDATE users SET " . implode(',', $setCols) . " WHERE id=?")->execute($setVals);
        redir($base . '?page=profile&saved=1');
    }
    $page = 'settings';
}
...
if ($act === 'change_password' && isLoggedIn() && checkCsrf()) {
    ...
    elseif ($nw !== $conf)                         $errors[] = 'Passwords do not match.';
        ...
        db()->prepare("UPDATE users SET password=? WHERE id=1")->execute([password_hash($nw, PASSWORD_DEFAULT)]);
        $info = 'Password updated successfully.';
    }
    $page = 'settings';
}

if ($act === 'refresh_feed' && isLoggedIn()) { refreshFeedCache(); jsonOut(['ok' => true]); }
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'feed') {
    ...
    $myAv     = ($cu && $cu['avatar']) ? $base . '?file=avatar' : '';
    ...
    foreach ($local as &$p) {
        $p['_src']      = 'local';
        $p['_key']      = 'local:' . $p['id'];
        $p['_author']   = $cu ? ['username' => $cu['username'], 'name' => $cu['name'], 'avatar' => $myAv] : [];
        $p['image_url'] = $p['image_path'] ? $base . '?file=post_img&id=' . $p['id'] : '';
        $p['post_url']  = $base . '?page=post&id=' . $p['id'];
        $p['_liked']    = false;
        if ($actor) {
            $ls = db()->prepare("SELECT COUNT(*) FROM likes WHERE post_id=? AND actor=?");
            $ls->execute([$p['id'], $actor]);
            $p['_liked'] = (bool) $ls->fetchColumn();
        }
        $cs = db()->prepare("SELECT COUNT(*) FROM comments WHERE post_id=?");
        $cs->execute([$p['id']]);
        $p['_comments'] = (int) $cs->fetchColumn();
        unset($p['image_path']);
    }
    unset($p);
    ...
    foreach ($rs->fetchAll() as $r) {
        ...
        $p = json_decode($r['post_json'], true);
        if (!$p) continue;
        $ts = (int) ($p['created_at'] ?? 0);
        if ($mode === 'after'  && $ts <= $after)  continue;
        if ($mode === 'before' && $ts >= $before) continue;
        if (!empty($r['follow_avatar'])) {
            if (!isset($p['author'])) $p['author'] = [];
            $p['author']['avatar'] = resolveAbsoluteUrl($r['follow_avatar'], $r['handle']);
        }
        if (isset($p['author']['avatar']) && $p['author']['avatar']) {
            $parsed = parseHandle($r['handle']);
            if ($parsed) $p['author']['avatar'] = resolveAbsoluteUrl($p['author']['avatar'], $parsed['url_https']);
        }
        $parsed            = parseHandle($r['handle']);
        $p['_src']         = 'remote';
    ...
    usort($arr, function ($a, $b) { return ($b['created_at'] ?? 0) <=> ($a['created_at'] ?? 0); });

    if ($mode === 'after') {
        jsonOut(['posts' => $arr, 'has_more' => false, 'is_refresh' => true]);
    } else {
        jsonOut(['posts' => array_slice($arr, 0, PPP), 'has_more' => count($arr) > PPP]);
    }
}
...
if ($page === 'post') {
    ...
    $s->execute([$pid]);
    $singlePost = $s->fetch();
    $s   = db()->prepare("SELECT * FROM comments WHERE post_id=? ORDER BY created_at ASC");
    $s->execute([$pid]);
    $comments = $s->fetchAll();
}
...
if ($page === 'remote_profile') {
    ...
    if ($remoteParsed) {
        ...
        if ($remoteProfileData && !empty($remoteProfileData['avatar'])) {
            $remoteProfileData['avatar'] = resolveAbsoluteUrl($remoteProfileData['avatar'], $remoteBase);
        }
        if ($activeTab === 'posts') {
            $rpd         = fedGet($remoteParsed, 'api=posts&limit=20');
            $remotePosts = $rpd['posts'] ?? [];
            foreach ($remotePosts as &$rp) {
                if (!empty($rp['author']['avatar'])) $rp['author']['avatar'] = resolveAbsoluteUrl($rp['author']['avatar'], $remoteBase);
            }
            unset($rp);
        } elseif ($activeTab === 'followers') {
            $rfd             = fedGet($remoteParsed, 'api=followers&limit=50');
            $remoteFollowers = $rfd['followers'] ?? [];
        }
    }
}
if ($page === 'remote_post') {
    $remoteHandle  = trim($_GET['handle'] ?? '');
    $remotePostId  = (int) ($_GET['id'] ?? 0);
    $remoteParsed  = $remoteHandle ? parseHandle($remoteHandle) : null;
    if ($remoteParsed && $remotePostId) {
        ...
        if ($remotePostData && !empty($remotePostData['author']['avatar'])) {
            $remotePostData['author']['avatar'] = resolveAbsoluteUrl($remotePostData['author']['avatar'], $remoteBase);
        }
    }
}
...
textarea{touch-action:manipulation;resize:vertical}
::-webkit-scrollbar{width:6px;height:6px}
::-webkit-scrollbar-track{background:transparent}
::-webkit-scrollbar-thumb{background:var(--bd);border-radius:3px}
.wrap{max-width:960px;margin:0 auto;padding:0 12px}
@media(min-width:600px){.wrap{padding:0 16px}}
.page-wrap{padding-top:8px;padding-bottom:calc(var(--mob-nav-h) + 16px)}
@media(min-width:768px){.page-wrap{padding-bottom:20px}}
.nav{background:var(--bg);border-bottom:1px solid var(--bd);position:sticky;top:0;z-index:100;height:var(--nav-h)}
.nav-inner{display:flex;align-items:center;gap:8px;height:100%;max-width:960px;margin:0 auto;padding:0 12px}
@media(min-width:600px){.nav-inner{padding:0 16px}}
.nav-logo{font-size:18px;font-weight:700;color:var(--tx);display:flex;align-items:center;gap:6px;flex-shrink:0;white-space:nowrap}
...
.nav-link.active{color:var(--blue);background:rgba(9,105,218,.06)}
...
.mob-nav{display:none;position:fixed;bottom:0;left:0;right:0;height:var(--mob-nav-h);background:var(--bg);border-top:1px solid var(--bd);z-index:100;padding-bottom:env(safe-area-inset-bottom,0px)}
.mob-nav-inner{display:flex;height:100%}
.mob-nav-item{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;color:var(--tx2);font-size:10px;font-weight:500;position:relative;text-decoration:none;min-width:0;padding:4px 2px;transition:color .15s}
.mob-nav-item:hover{text-decoration:none;color:var(--tx)}
...
.layout{display:grid;grid-template-columns:220px 1fr;gap:16px;padding:12px 0;align-items:start}
...
.sidebar-name{font-weight:700;font-size:15px;margin-top:6px}
.sidebar-uname{color:var(--tx2);font-size:12px}
.sidebar-bio{font-size:12px;color:var(--tx2);margin-top:6px;word-break:break-word}
...
.sidebar-nav-item:hover{background:var(--bg2);text-decoration:none}
.sidebar-nav-item.active{color:var(--blue);background:rgba(9,105,218,.06)}
.stat-item{display:flex;flex-direction:column;align-items:center;gap:1px;text-decoration:none;color:var(--tx);cursor:pointer}
.stat-item:hover .stat-num{text-decoration:underline}
.stat-num{font-size:16px;font-weight:700}
.stat-label{font-size:11px;color:var(--tx2)}
...
.compose-textarea{flex:1;border:1px solid var(--bd);border-radius:var(--r);padding:10px 12px;resize:vertical;min-height:76px;font-size:14px;transition:border-color .15s;width:0;min-width:0}
...
.char-count{font-size:12px;color:var(--tx2)}
...
.file-zone:hover,.file-zone.drag{border-color:var(--blue)}
.file-zone.show{display:block}
...
.post-header{display:flex;align-items:flex-start;gap:10px;margin-bottom:8px;flex-wrap:nowrap}
...
.post-content{font-size:14px;line-height:1.6;word-break:break-word;white-space:pre-wrap}
...
.action-btn:hover{background:var(--bg2);color:var(--tx);text-decoration:none}
...
.alert-warn{background:#fff8c5;color:var(--warn);border-color:#d4a72c}
.avatar{border-radius:50%;object-fit:cover;background:var(--bg3);flex-shrink:0;display:block}
...
.profile-cover{height:140px;background:linear-gradient(135deg,var(--blue),var(--purple));overflow:hidden}
@media(min-width:480px){.profile-cover{height:180px}}
@media(min-width:600px){.profile-cover{height:220px}}
.profile-cover img{width:100%;height:100%;object-fit:cover}
.profile-avatar-row{display:flex;align-items:flex-end;justify-content:space-between;padding:0 14px;margin-top:-30px;position:relative;z-index:2}
@media(min-width:480px){.profile-avatar-row{margin-top:-36px}}
@media(min-width:600px){.profile-avatar-row{padding:0 20px;margin-top:-44px}}
...
.profile-actions{padding-bottom:8px;display:flex;gap:6px;align-items:flex-end;flex-wrap:wrap}
.profile-info-section{padding:10px 14px 16px}
@media(min-width:600px){.profile-info-section{padding:12px 20px 20px}}
.profile-display-name{font-size:18px;font-weight:700}
@media(min-width:480px){.profile-display-name{font-size:20px}}
...
.profile-bio-text{font-size:14px;margin-top:8px;word-break:break-word}
...
.profile-stats{display:flex;gap:16px;margin-top:12px;padding-top:12px;border-top:1px solid var(--bd);flex-wrap:wrap}
@media(min-width:480px){.profile-stats{gap:24px}}
.page-tabs{display:flex;border-bottom:1px solid var(--bd);padding:0 10px;background:var(--bg);overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none}
.page-tabs::-webkit-scrollbar{display:none}
.page-tab{padding:12px 12px;font-size:14px;font-weight:500;color:var(--tx2);border-bottom:2px solid transparent;margin-bottom:-1px;text-decoration:none;white-space:nowrap;flex-shrink:0;transition:color .15s}
.page-tab:hover{color:var(--tx);text-decoration:none}
...
.follow-item-info{flex:1;min-width:120px}
.follow-handle{font-size:11px;font-family:monospace;color:var(--tx2);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.follow-item-actions{display:flex;gap:6px;flex-wrap:wrap;flex-shrink:0}
.notif-item{display:flex;align-items:flex-start;gap:10px;padding:12px 14px;border-bottom:1px solid var(--bd)}
.notif-item:last-child{border-bottom:none}
.notif-item.unread{background:rgba(9,105,218,.03)}
.comment-list{padding:0 14px}
.comment-item{display:flex;gap:10px;padding:10px 0;border-bottom:1px solid var(--bd)}
.comment-item:last-child{border-bottom:none}
.comment-body{flex:1;min-width:0}
.comment-author{font-weight:600;font-size:13px;color:var(--tx)}
.comment-author:hover{text-decoration:underline}
.comment-text{font-size:14px;margin-top:2px;white-space:pre-wrap;word-break:break-word}
.comment-time{font-size:11px;color:var(--tx2);margin-top:2px}
...
.login-card{width:100%;max-width:360px;background:var(--bg);border:1px solid var(--bd);border-radius:12px;padding:24px;box-shadow:var(--sh)}
@media(min-width:420px){.login-card{padding:28px}}
...
.link-item{display:flex;gap:8px}
...
<main class="login-wrap"><div class="login-card">
  ...
  <form method="POST" autocomplete="off">
    ...
    <div class="form-group">
      <label class="form-label" for="su">Username <span class="text-muted" style="font-weight:400">(permanent)</span></label>
      <input class="form-input" id="su" type="text" name="username" pattern="[a-z0-9_]{3,30}" placeholder="e.g. xsukax" required autofocus autocomplete="off">
...
<main class="login-wrap"><div class="login-card">
  ...
  <form method="POST">
    ...
    <div class="form-group"><label class="form-label" for="lu">Username</label><input class="form-input" id="lu" type="text" name="username" required autofocus autocomplete="username"></div>
    <div class="form-group"><label class="form-label" for="lp">Password</label><input class="form-input" id="lp" type="password" name="password" required autocomplete="current-password"></div>
  ...
  <aside class="sidebar">
    <div class="card">
      <div class="sidebar-profile">
        <div class="sidebar-cover"><?php if ($cu && $cu['cover']): ?><img src="<?= h($myCoverUrl) ?>" alt="Cover"><?php endif ?></div>
...
<div class="mt-12">
  <div class="card">
    <div class="profile-banner">
      <div class="profile-cover"><?php if ($cu && $cu['cover']): ?><img src="<?= h($myCoverUrl) ?>" alt="<?= h($cu['name']) ?> cover"><?php endif ?></div>
      <div class="profile-avatar-row">
        <div class="profile-avatar-ring"><?= avatarEl($myAvatarUrl, $cu['name'] ?? '', 76) ?></div>
    ...
    <?php else: foreach ($posts as $p):
        ...
        if (isLoggedIn()) {
            $ls = db()->prepare("SELECT COUNT(*) FROM likes WHERE post_id=? AND actor=?");
            $ls->execute([$p['id'], $actor]);
            $liked = (bool) $ls->fetchColumn();
        }
        $cs = db()->prepare("SELECT COUNT(*) FROM comments WHERE post_id=?");
        $cs->execute([$p['id']]);
        $cCount = (int) $cs->fetchColumn();
    ?>
      <article class="post" itemscope itemtype="https://schema.org/SocialMediaPosting">
        ...
        <div class="post-header">
          <?= avatarEl($myAvatarUrl, $cu['name'] ?? '', 38) ?>
          <div class="post-meta">
            <a class="post-author-name" href="<?= h($base . '?page=profile') ?>" itemprop="author"><?= h($cu['name'] ?? '') ?></a>
            <div class="post-author-handle">@<?= h($cu['username'] ?? '') ?></div>
          </div>
        ...
        <?php if ($p['image_path']): ?><div class="post-image"><img src="<?= h($base . '?file=post_img&id=' . $p['id']) ?>" loading="lazy" alt="Post image" itemprop="image"></div><?php endif ?>
        <div class="post-actions">
          ...
          <form method="POST" style="display:inline">
            ...
            <input type="hidden" name="_csrf" value="<?= h($csrf_token) ?>">
            <input type="hidden" name="post_id" value="<?= $p['id'] ?>">
            <button class="action-btn <?= $liked ? 'liked' : '' ?>">❤️ <?= $p['likes'] ?></button>
          </form>
    ...
    <?php if (!isLoggedIn()): ?>
    <div class="guest-cta">🔒 <a href="<?= h($base . '?page=login') ?>">Sign in</a> to interact with posts.</div>
    <?php endif ?>
  </div>
</div>
...
<div style="max-width:640px;margin-top:12px">
  <div class="card">
    ...
    <?php else: foreach ($followers as $f):
        $fst = db()->prepare("SELECT COUNT(*) FROM follows WHERE handle=?");
        $fst->execute([$f['handle']]);
        $alreadyFollowing = (bool) $fst->fetchColumn();
        $fAv = resolveAbsoluteUrl($f['avatar'] ?? '', $f['handle']);
    ?>
      <div class="follow-item">
        <?= avatarEl($fAv, $f['name'] ?: $f['handle'], 40) ?>
        <div class="follow-item-info">
          <div class="fw-bold text-sm truncate"><?= $f['name'] ? h($f['name']) : '—' ?></div>
          <div class="follow-handle truncate"><?= h($f['handle']) ?></div>
        </div>
        <div class="follow-item-actions">
          ...
          <?php if (!$alreadyFollowing): ?>
            <form method="POST">
              ...
              <input type="hidden" name="_csrf" value="<?= h($csrf_token) ?>">
              <input type="hidden" name="handle" value="<?= h($f['handle']) ?>">
              <input type="hidden" name="_back" value="<?= h($base . '?page=followers') ?>">
  ...
  <div class="card">
    ...
    ?>
      <div class="follow-item">
        <?= avatarEl($fAv, $f['name'] ?: $f['handle'], 40) ?>
        <div class="follow-item-info">
          <a class="fw-bold text-sm truncate" href="<?= h($base . '?page=remote_profile&handle=' . urlencode($f['handle'])) ?>" style="color:var(--tx);display:block"><?= $f['name'] ? h($f['name']) : '—' ?></a>
          <div class="follow-handle truncate"><?= h($f['handle']) ?></div>
        </div>
        <div class="follow-item-actions">
          <a class="btn btn-secondary btn-sm" href="<?= h($base . '?page=remote_profile&handle=' . urlencode($f['handle'])) ?>">View</a>
  ...
  <div class="card">
    ...
    <div class="settings-section">
      <form method="POST" enctype="multipart/form-data">
        ...
        <input type="hidden" name="_csrf" value="<?= h($csrf_token) ?>">
        <div class="form-group"><label class="form-label" for="sn">Display Name</label><input class="form-input" id="sn" type="text" name="name" value="<?= h($cu['name'] ?? '') ?>" maxlength="100" required></div>
        ...
        <div class="form-group"><label class="form-label" for="sb">Bio <span style="color:var(--tx2);font-weight:400">(max 300 chars)</span></label><textarea class="form-input" id="sb" name="bio" rows="3" maxlength="300"><?= h($cu['bio'] ?? '') ?></textarea></div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <?php if ($cu && $cu['avatar']): ?><img src="<?= h($myAvatarUrl) ?>" class="avatar mt-4" width="64" height="64" alt="Current avatar" style="margin-bottom:8px"><?php endif ?>
        ...
        <div class="form-group">
          ...
          <div class="cover-preview-box">
            <?php if ($cu && $cu['cover']): ?><img src="<?= h($myCoverUrl) ?>" alt="Cover" id="coverPreviewImg"><?php else: ?><img id="coverPreviewImg" style="display:none" alt="Cover preview"><?php endif ?>
  ...
  <div class="card mt-12">
    ...
    <div class="settings-section">
      <form method="POST">
        ...
        <input type="hidden" name="_csrf" value="<?= h($csrf_token) ?>">
        <div class="form-group"><label class="form-label" for="pc">Current password</label><input class="form-input" id="pc" type="password" name="current" required autocomplete="current-password"></div>
        <div class="form-group"><label class="form-label" for="pn">New password <span style="color:var(--tx2);font-weight:400">(min 8 chars)</span></label><input class="form-input" id="pn" type="password" name="newpw" required minlength="8" autocomplete="new-password"></div>
        <div class="form-group"><label class="form-label" for="pf">Confirm new password</label><input class="form-input" id="pf" type="password" name="confirm" required autocomplete="new-password"></div>
  ...
  <div class="card mt-12">
    ...
    <div class="settings-section">
      <div class="form-group">
        ...
        <div class="form-hint">Share this so others can follow you from their XFedi instance.</div>
      </div>
      <div class="form-group">
...
<div class="mt-12">
  ...
  <?php else:
    $rp        = $remoteProfileData;
    $rpAvatarUrl = $rp['avatar'] ?? '';
    $rpFollowers = (int) ($rp['followers_count'] ?? 0);
    $rpFollowing = (int) ($rp['following_count'] ?? 0);
    $stmtF = db()->prepare("SELECT COUNT(*) FROM follows WHERE handle=?");
    $stmtF->execute([$rp['handle'] ?? $remoteHandle]);
    $isFollowing = (bool) $stmtF->fetchColumn();
  ?>
    <div class="card">
      <div class="profile-banner">
        <div class="profile-cover"><?php if (!empty($rp['cover'])): ?><img src="<?= h($rp['cover']) ?>" alt="Cover" onerror="this.style.display='none'"><?php endif ?></div>
        <div class="profile-avatar-row">
            ...
            <?php if (isLoggedIn()): ?>
              <?php if ($isFollowing): ?>
                <form method="POST">
                  <input type="hidden" name="action" value="unfollow">
                  <input type="hidden" name="_csrf" value="<?= h($csrf_token) ?>">
                  <input type="hidden" name="handle" value="<?= h($rp['handle'] ?? $remoteHandle) ?>">
                  <input type="hidden" name="_back" value="<?= h($base . '?page=remote_profile&handle=' . urlencode($remoteHandle)) ?>">
      ...
      <?php if ($activeTab === 'posts'): ?>
        ...
        <?php else: foreach ($remotePosts as $rpost):
          $rpPostHandle = $rp['handle'] ?? $remoteHandle;
        ?>
          <article class="post">
            <div class="post-header">
              <?= avatarEl($rpAvatarUrl, $rp['name'] ?? '?', 38) ?>
              <div class="post-meta">
                <a class="post-author-name" href="<?= h($base . '?page=remote_profile&handle=' . urlencode($rpPostHandle)) ?>"><?= h($rp['name'] ?? 'Unknown') ?></a>
                <div class="post-author-handle"><?= h($rpPostHandle) ?></div>
              </div>
            ...
            <div class="post-actions">
              ...
              <a class="action-btn" href="<?= h($base . '?page=remote_post&handle=' . urlencode($rpPostHandle) . '&id=' . (int) $rpost['id']) ?>">💬 <?= (int) ($rpost['comment_count'] ?? 0) ?> · Open</a>
              <button class="action-btn share-btn" onclick="sharePost('<?= h($base . '?page=remote_post&handle=' . urlencode($rpPostHandle) . '&id=' . (int) $rpost['id']) ?>')">🔗</button>
            </div>
          </article>
        <?php endforeach; endif ?>
      <?php elseif ($activeTab === 'followers'): ?>
        ...
        <?php else: foreach ($remoteFollowers as $rf):
          $rfAv = resolveAbsoluteUrl($rf['avatar'] ?? '', $remoteBase);
        ?>
          <div class="follow-item">
            <?= avatarEl($rfAv, $rf['name'] ?: $rf['handle'], 40) ?>
            <div class="follow-item-info">
              <div class="fw-bold text-sm truncate"><?= $rf['name'] ? h($rf['name']) : '—' ?></div>
              <div class="follow-handle truncate"><?= h($rf['handle']) ?></div>
            </div>
              ...
              <div class="follow-item-actions">
                ...
                <?php if (!$alrF): ?>
                  <form method="POST">
                    <input type="hidden" name="action" value="follow">
                    <input type="hidden" name="_csrf" value="<?= h($csrf_token) ?>">
                    <input type="hidden" name="handle" value="<?= h($rf['handle']) ?>">
                    <input type="hidden" name="_back" value="<?= h($base . '?page=remote_profile&handle=' . urlencode($remoteHandle) . '&tab=followers') ?>">
                    <button class="btn btn-primary btn-sm">+ Follow</button>
                  </form>
                <?php else: ?><span class="remote-badge">✓ Following</span><?php endif ?>
              </div>
...
<?php elseif ($page === 'post' && $singlePost):
  $p     = $singlePost;
  $actor = 'local:' . ($cu['username'] ?? '');
  $liked = false;
  if (isLoggedIn()) {
  ...
  $cs2    = db()->prepare("SELECT COUNT(*) FROM comments WHERE post_id=?");
  $cs2->execute([$p['id']]);
  $cCount = (int) $cs2->fetchColumn();
?>
<div style="max-width:640px;margin-top:12px">
  <div style="margin-bottom:10px"><a href="<?= h($base) ?>" class="btn btn-secondary btn-sm">← Back</a></div>
  <div class="card">
    <article class="post" itemscope itemtype="https://schema.org/SocialMediaPosting">
      ...
      <div class="post-header">
        ...
        <div class="post-meta">
          <a class="post-author-name" href="<?= h($base . '?page=profile') ?>" itemprop="author"><?= h($cu['name'] ?? '') ?></a>
          <div class="post-author-handle">@<?= h($cu['username'] ?? '') ?> · <?= timeEl($p['created_at']) ?></div>
        </div>
        <?php if (isLoggedIn()): ?>
        <div class="post-owner-actions">
      ...
      <div class="post-content" itemprop="text"><?= linkify($p['content']) ?></div>
      <?php if ($p['image_path']): ?><div class="post-image"><img src="<?= h($base . '?file=post_img&id=' . $p['id']) ?>" alt="Post image" itemprop="image"></div><?php endif ?>
      <div class="post-actions">
        <?php if (isLoggedIn()): ?>
        <form method="POST" style="display:inline">
          <input type="hidden" name="action" value="<?= $liked ? 'unlike' : 'like' ?>">
    ...
    <section class="comment-list">
      <?php foreach ($comments as $c):
        $isLocal  = !$c['is_remote'];
        $cAvUrl   = $isLocal ? $myAvatarUrl : '';
        $cDisplay = $c['author'];
        $cUrl     = $isLocal ? $base . '?page=profile' : '';
        if (!$isLocal && preg_match('/^@[^@\s]+@[^\s]+$/', $cDisplay)) {
            $fq = db()->prepare("SELECT avatar FROM follows WHERE handle=?");
            $fq->execute([$cDisplay]);
            $fr = $fq->fetch();
            $cAvUrl = resolveAbsoluteUrl($fr['avatar'] ?? '', $cDisplay);
            $cUrl   = $base . '?page=remote_profile&handle=' . urlencode($cDisplay);
        }
      ?>
        <div class="comment-item">
          <?= avatarEl($cAvUrl, $cDisplay, 32) ?>
          <div class="comment-body">
            <?php if ($cUrl): ?><a class="comment-author" href="<?= h($cUrl) ?>"><?= h($cDisplay) ?></a>
            <?php else: ?><span class="comment-author"><?= h($cDisplay) ?></span><?php endif ?>
            <?php if ($c['is_remote']): ?><span class="remote-badge" style="margin-left:4px">remote</span><?php endif ?>
...
<div class="remote-post-wrap mt-12">
  ...
  <?php else:
    $rp2        = $remotePostData;
    $rpAuth     = $remoteProfileData ?? [];
    $rpAvatarUrl = $rpAuth['avatar'] ?? '';
    $rpAuthorUsername = $rpAuth['username'] ?? null;
  ?>
    <div class="card">
      <article class="post">
        <div class="post-header">
          ...
          <div class="post-meta">
            <a class="post-author-name" href="<?= h($base . '?page=remote_profile&handle=' . urlencode($remoteHandle)) ?>"><?= h($rpAuth['name'] ?? 'Unknown') ?></a>
            <div class="post-author-handle"><?= h($remoteHandle) ?> · <?= timeEl((int) ($rp2['created_at'] ?? 0)) ?></div>
          </div>
        </div>
      ...
      <section id="remoteCommentList" class="comment-list">
        <?php
        $rpFullHandle   = $remoteProfileData['handle'] ?? $remoteHandle;
        $rcAvatarCache  = [];
        if (!empty($rp2['comments'])): foreach ($rp2['comments'] as $rc):
          $rcDisplay     = $rc['author'] ?? 'Unknown';
          $rcIsFedHandle = (bool) preg_match('/^@[^@\s]+@[^\s]+$/', $rcDisplay);
          $rcIsAuthor    = ($rcDisplay === $rpAuthorUsername) || ($rcDisplay === $rpFullHandle);
          $rcAvatar      = '';
          if ($rcIsAuthor) {
              $rcAvatar = $rpAvatarUrl;
          } elseif ($rcIsFedHandle) {
              if (isset($rcAvatarCache[$rcDisplay])) {
                  $rcAvatar = $rcAvatarCache[$rcDisplay];
              } else {
                  $fq = db()->prepare("SELECT avatar FROM follows WHERE handle=?");
                  $fq->execute([$rcDisplay]);
                  $fr = $fq->fetch();
                  if ($fr && $fr['avatar']) {
                      $rcAvatar = resolveAbsoluteUrl($fr['avatar'], $rcDisplay);
                  } else {
                      $rcParsed = parseHandle($rcDisplay);
                      if ($rcParsed) {
                          $rcProf   = fedGet($rcParsed, 'api=profile');
                          $rcAvatar = $rcProf ? resolveAbsoluteUrl($rcProf['avatar'] ?? '', $rcParsed['url_https']) : '';
                      }
                  }
                  $rcAvatarCache[$rcDisplay] = $rcAvatar;
              }
          } else {
              $rcAvatar = $rpAvatarUrl;
          }
          $rcProfileUrl = $rcIsFedHandle
              ? $base . '?page=remote_profile&handle=' . urlencode($rcDisplay)
              : $base . '?page=remote_profile&handle=' . urlencode($remoteHandle);
        ?>
          <div class="comment-item">
            <?= avatarEl($rcAvatar, $rcDisplay, 30) ?>
            <div class="comment-body">
              <a class="comment-author" href="<?= h($rcProfileUrl) ?>"><?= h($rcDisplay) ?></a>
              <?php if (!empty($rc['is_remote'])): ?><span class="remote-badge" style="margin-left:4px">remote</span><?php endif ?>
      ...
      <?php else: ?>
      <div class="guest-cta">🔒 <a href="<?= h($base . '?page=login') ?>">Sign in</a> to interact.</div>
      <?php endif ?>
    </div>
  <?php endif ?>
</div>
...
<div class="modal-overlay" id="editModal" role="dialog" aria-modal="true">
  <div class="modal">
    ...
    <form method="POST">
      ...
      <div class="form-group">
        <textarea class="form-input" name="content" id="editContent" rows="4" maxlength="<?= MAX_CHARS ?>" oninput="updateEditCount()"></textarea>
        <div class="form-hint text-sm" id="editCharCount">0 / <?= MAX_CHARS ?></div>
      </div>
...
function updateCharCount(){var n=document.getElementById('composeText');var el=document.getElementById('charCount');if(!n||!el)return;var l=n.value.length;el.textContent=l+' / '+MAX;el.className='char-count'+(l>MAX*.85?' warn':'')+(l>=MAX?' over':'');}
function switchPostType(t){document.getElementById('postTypeInput').value=t;document.getElementById('tab-text').classList.toggle('active',t==='text');document.getElementById('tab-image').classList.toggle('active',t==='image');var z=document.getElementById('imageUploadZone');if(z)z.classList.toggle('show',t==='image');}
function previewImg(inp){if(!inp.files||!inp.files[0])return;var r=new FileReader();r.onload=function(e){var p=document.getElementById('imgPreview');if(p){p.src=e.target.result;p.style.display='block';}};r.readAsDataURL(inp.files[0]);}
function handleImgDrop(e){e.preventDefault();var z=document.getElementById('imageUploadZone');if(z)z.classList.remove('drag');if(e.dataTransfer&&e.dataTransfer.files.length){var dt=new DataTransfer();dt.items.add(e.dataTransfer.files[0]);var inp=document.getElementById('imgFile');inp.files=dt.files;previewImg(inp);}}
function previewCover(inp){if(!inp.files||!inp.files[0])return;var r=new FileReader();r.onload=function(e){var img=document.getElementById('coverPreviewImg');if(img){img.src=e.target.result;img.style.display='block';}};r.readAsDataURL(inp.files[0]);}
...
function updateEditCount(){var n=document.getElementById('editContent');var el=document.getElementById('editCharCount');if(n&&el)el.textContent=n.value.length+' / '+MAX;}
var editModal=document.getElementById('editModal');if(editModal)editModal.addEventListener('click',function(e){if(e.target===e.currentTarget)closeEditModal();});
function toast(msg,bg){var t=document.getElementById('toast');if(!t)return;t.textContent=msg;t.style.background=bg||'#1f2328';t.classList.add('show');setTimeout(function(){t.classList.remove('show');},3000);}
function toastOk(m){toast(m,'#1a7f37');}
function toastErr(m){toast(m,'#cf222e');}
function sharePost(url){if(navigator.share)navigator.share({url:url});else navigator.clipboard.writeText(url).then(function(){toast('Link copied!');});}
function copyHandle(){navigator.clipboard.writeText(HNDL).then(function(){toast('Handle copied!');});}
function addLinkField(){var l=document.getElementById('linkList');if(!l)return;var d=document.createElement('div');d.className='link-item';d.innerHTML='<input class="form-input" type="url" name="links[]" placeholder="https://…" style="min-width:0"><button type="button" class="btn btn-secondary btn-sm" onclick="this.parentNode.remove()">✕</button>';l.appendChild(d);}

function esc(s){return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;');}
function agoJS(ts){var d=Math.max(0,Math.floor(Date.now()/1000)-ts);if(d<60)return d+'s';if(d<3600)return Math.floor(d/60)+'m';if(d<86400)return Math.floor(d/3600)+'h';if(d<604800)return Math.floor(d/86400)+'d';return new Date(ts*1000).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'});}
function timeElJS(ts){var iso=new Date(ts*1000).toISOString();var title=new Date(ts*1000).toLocaleString('en-US',{month:'long',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit'});return'<time datetime="'+iso+'" title="'+title+'">'+agoJS(ts)+'</time>';}
function linkifyJS(t){return String(t).split(/(https?:\/\/\S+)/gi).map(function(p,i){if(i%2===0)return p.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');var u=p.replace(/[.,;:!?)'"\}>]+$/,'');var d=u.length>55?u.slice(0,52)+'…':u;return'<a href="'+u.replace(/"/g,'&quot;')+'" target="_blank" rel="noopener noreferrer" style="word-break:break-all">'+esc(d)+'</a>';}).join('');}
function avatarElJS(url,name,size){var init=esc(((name||'?').charAt(0)).toUpperCase());var fs=Math.floor(size*.4);var st='width:'+size+'px;height:'+size+'px;';var ph='<div class=\'avatar-placeholder\' style=\''+st+'font-size:'+fs+'px\'>'+init+'</div>';if(url){return'<img src="'+esc(url)+'" class="avatar" style="'+st+'" alt="'+esc(name)+'" loading="lazy" onerror="this.outerHTML=\''+ph.replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\'">';}return'<div class="avatar-placeholder" style="'+st+'font-size:'+fs+'px" aria-label="'+esc(name)+'">'+init+'</div>';}

function remoteInteractBtn(btn,type,remoteBase,postId,actor){
  btn.classList.add('interaction-loading');
  var span=btn.querySelector('span');
  var urls=[remoteBase,remoteBase.replace(/^https/,'http').replace(/^http:/,'https:')];
  var tried=0;
  function tryNext(){
    if(tried>=urls.length){btn.classList.remove('interaction-loading');toastErr('Could not reach remote server.');return;}
    var url=urls[tried++];
    fetch(url+'?api=interact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:type,post_id:parseInt(postId),actor:actor})}).then(function(r){return r.json();}).then(function(d){if(d.ok){if(span&&type==='like'&&d.likes!==undefined)span.textContent=d.likes;btn.classList.remove('interaction-loading');toastOk(type==='like'?'Liked! ❤️':'Done!');} else{tryNext();}}).catch(function(){tryNext();});
  }
  tryNext();
}

<?php if ($page === 'remote_post' && !empty($remoteBase) && !empty($remotePostData) && !isset($remotePostData['error'])): ?>
var REMOTE_BASE        = <?= json_encode($remoteBase) ?>;
var REMOTE_POST_ID     = <?= (int) ($remotePostData['id'] ?? 0) ?>;
var REMOTE_HANDLE      = <?= json_encode($remoteHandle) ?>;
var REMOTE_AVATAR      = <?= json_encode($remoteProfileData['avatar'] ?? '') ?>;
var REMOTE_AUTH_UNAME  = <?= json_encode($remoteProfileData['username'] ?? null) ?>;
var REMOTE_FULL_HANDLE = <?= json_encode($remoteProfileData['handle'] ?? $remoteHandle) ?>;
var REMOTE_AVATAR_CACHE= <?= json_encode($rcAvatarCache ?? []) ?>;

function remotePostLike(btn){
  btn.classList.add('interaction-loading');
  var cnt=document.getElementById('remLikeCount');
  fetch(REMOTE_BASE+'?api=interact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'like',post_id:REMOTE_POST_ID,actor:HNDL})}).then(function(r){return r.json();}).then(function(d){if(d.ok){if(cnt&&d.likes!==undefined)cnt.textContent=d.likes;btn.classList.add('liked');}btn.classList.remove('interaction-loading');toastOk('Liked! ❤️');}).catch(function(){btn.classList.remove('interaction-loading');toastErr('Could not reach remote server.');});
}

function submitRemoteComment(){
  var text=document.getElementById('remoteCommentText').value.trim();
  var errEl=document.getElementById('remoteCommentError');
  var btn=document.getElementById('remoteCommentBtn');
  if(errEl)errEl.style.display='none';
  if(!text){if(errEl){errEl.textContent='Comment cannot be empty.';errEl.style.display='block';}return;}
  btn.disabled=true;btn.textContent='Posting…';
  fetch(REMOTE_BASE+'?api=interact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'comment',post_id:REMOTE_POST_ID,actor:HNDL,content:text})}).then(function(r){return r.json();}).then(function(d){if(d.ok){document.getElementById('remoteCommentText').value='';toastOk('Comment posted! ✓');refreshRemoteComments();}else{if(errEl){errEl.textContent='Failed. Remote server returned an error.';errEl.style.display='block';}}}).catch(function(){if(errEl){errEl.textContent='Failed. Remote server may be unreachable.';errEl.style.display='block';}}).finally(function(){btn.disabled=false;btn.textContent='Post comment';});
}

function refreshRemoteComments(){
  fetch(REMOTE_BASE+'?api=post&id='+REMOTE_POST_ID).then(function(r){return r.json();}).then(function(d){if(!d.error){renderRemoteComments(d.comments||[]);var lc=document.getElementById('remLikeCount');if(lc)lc.textContent=d.likes||0;var cc=document.getElementById('remCommentCount');if(cc)cc.textContent=(d.comments||[]).length;}}).catch(function(){});
}

function renderRemoteComments(comments){
  var el=document.getElementById('remoteCommentList');if(!el)return;
  var profileUrl=BASE+'?page=remote_profile&handle='+encodeURIComponent(REMOTE_HANDLE);
  el.innerHTML=comments.map(function(c){
    var cDisplay=c.author||'Unknown';
    var isAuthor=(REMOTE_AUTH_UNAME&&cDisplay===REMOTE_AUTH_UNAME)||(cDisplay===REMOTE_FULL_HANDLE);
    var isFed=/^@[^@\s]+@[^\s]+$/.test(cDisplay);
    var cAvatar=isAuthor?REMOTE_AVATAR:(REMOTE_AVATAR_CACHE[cDisplay]||(!isFed?REMOTE_AVATAR:''));
    var cUrl=isFed?BASE+'?page=remote_profile&handle='+encodeURIComponent(cDisplay):profileUrl;
    return'<div class="comment-item">'+avatarElJS(cAvatar,cDisplay,30)+'<div class="comment-body"><a class="comment-author" href="'+esc(cUrl)+'">'+esc(cDisplay)+'</a>'+(c.is_remote?'<span class="remote-badge" style="margin-left:4px">remote</span>':'')+'<div class="comment-text">'+linkifyJS(c.content||'')+'</div><div class="comment-time">'+timeElJS(c.created_at||0)+'</div></div></div>';
  }).join('');
}
<?php endif ?>

<?php if ($page === 'home'): ?>
var feedBefore   = 9999999999;
var feedNewest   = 0;
var feedLoading  = false;
var feedHasMore  = true;
var renderedKeys = {};

var MY_AVATAR = <?= json_encode($myAvatarUrl) ?>;
var MY_NAME   = <?= json_encode($cu['name'] ?? '') ?>;
var MY_UNAME  = <?= json_encode($cu['username'] ?? '') ?>;

function postKey(p){return p._key||((p._src==='local'?'local':(p._handle||'?'))+':'+p.id);}

function renderPost(p){
  var src    = p._src||'remote';
  var author = p._author||p.author||{};
  var name   = src==='local'?MY_NAME :(author.name||(p._handle?p._handle.split('@')[1]||'Remote':'Remote'));
  var uname  = src==='local'?MY_UNAME:(author.username||'');
  var avUrl  = src==='local'?MY_AVATAR:(author.avatar||'');
  var profUrl= src==='local'?(BASE+'?page=profile'):(p._profile_url||'');
  var postUrl= p.post_url||(src==='local'?BASE+'?page=post&id='+p.id:'#');
  var imgHtml= p.image_url?'<div class="post-image"><img src="'+esc(p.image_url)+'" loading="lazy" alt="Post image" onerror="this.parentNode.style.display=\'none\'"></div>':'';
  var remBadge=src==='remote'?'<a href="'+esc(profUrl)+'" class="remote-badge" style="margin-bottom:6px;display:inline-flex;text-decoration:none;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">📡 '+esc(p._handle||'remote')+'</a>':'';
  var edited =(p.updated_at&&p.updated_at>p.created_at)?'<span class="remote-badge">edited</span>':'';
  var nameEl =profUrl?'<a class="post-author-name" href="'+esc(profUrl)+'">'+esc(name)+'</a>':'<span class="post-author-name">'+esc(name)+'</span>';
  var cCount = src==='local'?(p._comments||0):(p.comment_count||(p.comments?p.comments.length:0));
  var likeHtml;
  if(src==='local'&&LOGGED_IN){likeHtml='<form method="POST" style="display:inline"><input type="hidden" name="action" value="'+(p._liked?'unlike':'like')+'"><input type="hidden" name="_csrf" value="'+CSRF+'"><input type="hidden" name="post_id" value="'+p.id+'"><button class="action-btn'+(p._liked?' liked':'')+'" >❤️ '+(p.likes||0)+'</button></form>';}
  else if(src==='remote'&&LOGGED_IN){var rb=p._remote_base||'';likeHtml='<button class="action-btn" onclick="remoteInteractBtn(this,\'like\',\''+esc(rb)+'\',\''+p.id+'\',\''+esc(HNDL)+'\')">❤️ <span>'+(p.likes||0)+'</span></button>';}
  else{likeHtml='<span class="action-stat">❤️ '+(p.likes||0)+'</span>';}
  var commentHtml='<a class="action-btn" href="'+esc(postUrl)+'">💬 '+cCount+(src==='remote'?' · Open':'')+'</a>';
  var shareHtml='<button class="action-btn share-btn" onclick="sharePost(\''+esc(postUrl)+'\')">🔗</button>';
  return'<article class="post" data-post-key="'+esc(postKey(p))+'" data-ts="'+(p.created_at||0)+'">'
    +remBadge
    +'<div class="post-header">'+avatarElJS(avUrl,name,38)+'<div class="post-meta">'+nameEl+'<div class="post-author-handle">'+(uname?'@'+esc(uname):esc(p._handle||''))+'</div></div><span class="post-time">'+timeElJS(p.created_at||0)+'</span>'+edited+'</div>'
    +'<div class="post-content">'+linkifyJS(p.content||'')+'</div>'+imgHtml
...
function loadFeed(append){
  ...
  fetch('?ajax=feed&mode=before&before='+feedBefore).then(function(r){return r.json();}).then(function(data){
    var container=document.getElementById('feedContainer');if(!container){feedLoading=false;return;}
    ...
    if(!posts.length&&!append){
      ...
      posts.forEach(function(p){
        var k=postKey(p);if(renderedKeys[k])return;
        renderedKeys[k]=true;
        container.insertAdjacentHTML('beforeend',renderPost(p));
        added++;
        var ts=p.created_at||0;
        if(ts<feedBefore)feedBefore=ts-1;
        if(ts>feedNewest)feedNewest=ts;
      });
      feedHasMore=!!data.has_more;
      var total=Object.keys(renderedKeys).length;
      if(status)status.textContent=total+' post'+(total!==1?'s':'')+' loaded'+(feedHasMore?'':' · all loaded');
    }
    if(lmWrap)lmWrap.style.display=feedHasMore?'block':'none';
    if(lmBtn)lmBtn.textContent='Load 20 more';
  }).catch(function(){var s=document.getElementById('feedStatus');if(s)s.textContent='Could not load feed.';}).finally(function(){feedLoading=false;if(spinner)spinner.style.display='none';});
}
...
function checkNewPosts(){
  ...
  feedLoading=true;
  var btn=document.getElementById('checkNewBtn');
  if(btn){btn.disabled=true;btn.textContent='⏳ Checking…';}
  fetch('?ajax=feed&mode=after&after='+feedNewest).then(function(r){return r.json();}).then(function(data){
    var posts=data.posts||[];var container=document.getElementById('feedContainer');
    var added=0;
    posts.sort(function(a,b){return(a.created_at||0)-(b.created_at||0);});
    posts.forEach(function(p){
      var k=postKey(p);if(renderedKeys[k])return;
      renderedKeys[k]=true;
      container.insertAdjacentHTML('afterbegin',renderPost(p));
      added++;
      var ts=p.created_at||0;if(ts>feedNewest)feedNewest=ts;
    });
    var status=document.getElementById('feedStatus');
    if(status)status.textContent=added?added+' new post'+(added!==1?'s':'')+' added.':'Feed is up to date.';
    if(btn)btn.style.display='none';
  }).catch(function(){}).finally(function(){feedLoading=false;if(btn){btn.disabled=false;btn.textContent='⬆ New posts';}});
}

function syncAndRefresh(){
  if(!LOGGED_IN)return;
  var btn=document.getElementById('refreshBtn');var status=document.getElementById('feedStatus');
  if(btn){btn.disabled=true;btn.textContent='⏳ Syncing…';}
  if(status)status.textContent='Syncing followed instances…';
  fetch('?',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=refresh_feed&_csrf='+CSRF}).then(function(){return checkNewPosts();}).catch(function(){if(status)status.textContent='Sync failed.';}).finally(function(){if(btn){btn.disabled=false;btn.textContent='🔄 Sync';}});
}

setInterval(function(){
  if(feedLoading||!feedNewest)return;
  fetch('?ajax=feed&mode=after&after='+feedNewest).then(function(r){return r.json();}).then(function(data){
    var count=(data.posts||[]).filter(function(p){return!renderedKeys[postKey(p)];}).length;
    var btn=document.getElementById('checkNewBtn');
